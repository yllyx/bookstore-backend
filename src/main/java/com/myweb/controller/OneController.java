...
public class OneController
{
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "isbn", value = "isbn（必需）", required = true,
        dataType = "String")})
    @ResponseBody
    ...
    @ApiImplicitParams({
        ...
        @ApiImplicitParam(name = "userid", value = "userid（必需）", required = true,
            dataType = "Integer")})
    @ResponseBody
    ...
    @ApiImplicitParams({
        @ApiImplicitParam(name = "bookids", value = "bookids（必需）,这个id是book的id,多选以逗号隔开，如1,2,3,4",
            required = true, dataType = "String"),
        @ApiImplicitParam(name = "ownerid", value = "ownerid（必需)，这个id是user的id", required = true,
            dataType = "Integer")})
    @ResponseBody
    ...
    @ApiImplicitParams({
        @ApiImplicitParam(name = "ownerid", value = "ownerid（必需）,这个id是user的id", required = true,
            dataType = "Integer"),
        @ApiImplicitParam(name = "status", value = "status（可选）,0为全部（不传此参数时的默认值），1为自有，2为借出，3为借入",
    ...
    @ApiImplicitParams({
        @ApiImplicitParam(name = "ownerid", value = "ownerid（必需）,这个id是user的id", required = true,
            dataType = "Integer"),
        @ApiImplicitParam(name = "bookid", value = "bookid（必需）,这个id是book的id", required = true,
            dataType = "Integer"),
        @ApiImplicitParam(name = "days", value = "归还天数（可选）,整数", required = true,
            dataType = "Integer"),
        @ApiImplicitParam(name = "fee", value = "借阅费用（可选）,2位小数", required = true,
            dataType = "BigDecimal"),
        @ApiImplicitParam(name = "deposit", value = "押金数额（可选）,2位小数", required = true,
            dataType = "BigDecimal"),
        @ApiImplicitParam(name = "weight", value = "单位（可选，默认为0）0天，1周，2月", required = true,
            dataType = "BigDecimal")})
    @ResponseBody
    ...
    @ApiImplicitParams({
        @ApiImplicitParam(name = "username", value = "用户名（必需）", required = true,
            dataType = "String"),
        @ApiImplicitParam(name = "password", value = "密码（必需）", required = true,
            dataType = "String")})
    @ResponseBody
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "code", value = "code（必需）", required = true,
        dataType = "String"),})
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "openid", value = "openid（必需）", required = true,
        dataType = "String"),})
    ...
    @ApiOperation(value = "获取微信用户信息", notes = "只有关注了公众号的用户才能获取到详细的用户信息")
    @ApiImplicitParams({@ApiImplicitParam(name = "openid", value = "openid（必需）", required = true,
        dataType = "String"),})
    @GetMapping("user/weixin/userInfo")
    @ResponseBody
    public Result weixinUserInfo(@ModelAttribute OneParameter oneParameter)
    {
        return oneService.weixinUserInfo(oneParameter);
    }
}
