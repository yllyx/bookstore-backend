...
import com.utils.wechat.WeChatUtil;
...
public class OneServiceImpl implements OneService
{
    ...
    public Result scan(OneParameter oneParameter)
    {
        ...
        {
            book.setPrice(
                new BigDecimal(dbBook.getPrice().substring(0, dbBook.getPrice().length() - 1)));
        }
    ...
    public Result list(OneParameter oneParameter)
    {
        ...
        {
            bookstoreList = bookstoreRepository.findAllByOwneridOrUserid(oneParameter.getOwnerid(),
                oneParameter.getOwnerid());
        }
        else if (oneParameter.getStatus() == 1 || oneParameter.getStatus() == 2)
        {
            bookstoreList = bookstoreRepository.findAllByOwneridAndStatus(
                oneParameter.getOwnerid(), oneParameter.getStatus());
        }
        else if (oneParameter.getStatus() == 3)
        {
            bookstoreList =
                bookstoreRepository.findAllByUseridAndStatus(oneParameter.getOwnerid(), 2);
        }
    ...
    public Result out(OneParameter oneParameter)
    {
        Result result = new Result();
        if (oneParameter.getOwnerid() == null || oneParameter.getOwnerid() == 0
            || StringUtils.isBlank(oneParameter.getBookids()))
        {
            result.setMessage("必须的参数不能为空!");
            return result;
        }
        ...
        List<Bookstore> bookstoreList = bookstoreRepository.findAllByBookidsAndOwneridAndStatus(
            bookids, oneParameter.getOwnerid(), 1);
        if (bookstoreList.size() > 0)
        {
    ...
    {
        Result result = new Result();
        if (StringUtils.isBlank(oneParameter.getUsername())
            || StringUtils.isBlank(oneParameter.getPassword()) || oneParameter.getId() != 0)
        {
            result.setMessage("必须的参数不能为空!");
            return result;
        }
    ...
    public Result login(OneParameter oneParameter)
    {
        Result result = new Result();
        if (StringUtils.isBlank(oneParameter.getUsername())
            || StringUtils.isBlank(oneParameter.getPassword()))
        {
            result.setMessage("必须的参数不能为空!");
            return result;
        }
        List<User> userList = userRepository.findByUsernameAndPassword(oneParameter.getUsername(),
            oneParameter.getPassword());
        if (userList.size() == 1)
        {
            result.setStatus(1);
            result.setData(userList.get(0));
    ...
    public Result set(OneParameter oneParameter)
    {
        Result result = new Result();
        if (oneParameter.getOwnerid() == null || oneParameter.getOwnerid() == 0
            || oneParameter.getBookid() == null || oneParameter.getBookid() == 0)
        {
            result.setMessage("必须的参数不能为空!");
            return result;
        }
        List<Bookstore> bookstoreList = bookstoreRepository.findAllByBookidAndOwneridAndStatus(
            oneParameter.getBookid().intValue(), oneParameter.getOwnerid().intValue(), 1);
        if (bookstoreList.size() > 0)
        {
            bookstoreList.forEach(e -> {
                if (oneParameter.getDeposit() != null)
                    e.setDeposit(oneParameter.getDeposit());
                if (oneParameter.getDays() != null)
                    e.setDays(oneParameter.getDays());
                if (oneParameter.getFee() != null)
                    e.setFee(oneParameter.getFee());
    ...
    public Result weixinUserInfo(OneParameter oneParameter)
    {
        ...
        try
        {
            // 首先根据appId,appSecret获取access_token
            String accessToken = weChatUtil.getToken(appId, appSecret).getAccessToken();

            // 根据用户openId和accessToken信息获取用户信息
            WeChatUserInfo userInfo =
                weChatUtil.getUserInfo(accessToken, oneParameter.getOpenid());
        ...
        {
            result.setStatus(CommonConst.FAIL);
            result.setMessage("获取用户信息异常：" + e.getMessage());
        }
        return result;
    }
}
